...
void Game::initialize()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(m_vertex) * 36, vertex, GL_STATIC_DRAW);
...
void Game::update()
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
	{
		for (int i = 0; i < 36; i++)
		{
			cubeFace = { vertex[i].coordinate[0],vertex[i].coordinate[1] ,vertex[i].coordinate[2] };
			cubeFace = MyMatrix3::rotationX(0.25) * cubeFace;

			vertex[i].coordinate[0] = cubeFace.x;
			vertex[i].coordinate[1] = cubeFace.y;
			vertex[i].coordinate[2] = cubeFace.z;
		}
	}
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A))
	{
		...
		{
			cubeFace = { vertex[i].coordinate[0],vertex[i].coordinate[1] ,vertex[i].coordinate[2] };
			cubeFace = MyMatrix3::rotationY(0.25) * cubeFace;
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
	{
		...
		{
			cubeFace = { vertex[i].coordinate[0],vertex[i].coordinate[1] ,vertex[i].coordinate[2] };
			cubeFace = MyMatrix3::scale(1.01) * cubeFace;
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right))
	{
		...
		{
			cubeFace = { vertex[i].coordinate[0],vertex[i].coordinate[1] ,vertex[i].coordinate[2] };
			cubeFace = MyMatrix3::scale(0.99) * cubeFace;
...
void Game::render()
{
	...
	glColorPointer(3, GL_FLOAT, sizeof(m_vertex), (char*)NULL + 12);

	/*	Draw Triangle from VBO	(set where to start from as VBO can contain 
		model compoents that are and are not to be drawn )	*/
	glVertexPointer(3, GL_FLOAT, sizeof(m_vertex), (char*)NULL + 0);
